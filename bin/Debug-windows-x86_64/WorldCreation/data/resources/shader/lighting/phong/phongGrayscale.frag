...
vec4 colorToGrayscale(vec4 initialColor)
{
	//Grayscale values from https://kidscancode.org/godot_recipes/shaders/greyscale/
    float average = (initialColor.r + initialColor.g + initialColor.b) / 3.0;

	return vec4(average, average, average, initialColor.a);
}
...
void main()
{
	...
	if (bool(usingTexture))
	{
		...
		texColor =  colorToGrayscale(texColor);
	...
	{
		fragmentColor = colorToGrayscale(vertColor); 
	}
}