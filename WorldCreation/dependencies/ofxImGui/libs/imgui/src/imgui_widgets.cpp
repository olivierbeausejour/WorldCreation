...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags, bool updateFromWidget)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            else
            {
                ...
                {
                    if (updateFromWidget)
                        hueValueEntered = round(359.0f * (i[0] / 255.0f));
                        
                    i[0] = std::min(359.0f, (float)hueValueEntered);
                    i[1] = std::min(100.0f, round(100.0f * (i[1] / 255.0f)));
                    i[2] = std::min(100.0f, round(100.0f * (i[2] / 255.0f)));

                    if (n == 0)
                        value_changed |= DragInt(ids[n], &i[n], 0.5f, 0, hdr ? 0 : 359, fmt_table_int[fmt_idx][n]);
                    else
                        value_changed |= DragInt(ids[n], &i[n], 1.0f, 0, hdr ? 0 : 100, fmt_table_int[fmt_idx][n]);

                    if (value_changed && i[0] != hueValueEntered || updateFromWidget)
                        hueValueEntered = i[0];
                    
                    if (i[0] > 359)
                        i[0] = 0;

                    i[0] = std::min(255.0f, round(255.0f * (i[0] / 359.0f)));
                    i[1] = std::min(255.0f, round(255.0f * (i[1] / 100.0f)));
                    i[2] = std::min(255.0f, round(255.0f * (i[2] / 100.0f)));
                }
                else
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayHSV || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            value_changed |= ColorEdit4("##hsv", col, sub_flags | ImGuiColorEditFlags_DisplayHSV, value_changed_h);
